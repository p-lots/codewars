...
std::string Stat::stat(const std::string &strng)
{
    ...
    Time min = *(std::min_element(times.begin(), times.end(), 
        [](const Time &lhs, const Time &rhs) {
            if (lhs.h < rhs.h) return true;
            else if (lhs.h > rhs.h) return false;
            else {
    ...
    Time max = *(std::max_element(times.begin(), times.end(),
            [](const Time &lhs, const Time &rhs) {
            if (lhs.h < rhs.h) return true;
            else if (lhs.h > rhs.h) return false;
            else {
    ...
    ret_ss << "Range: " << std::setw(2) << std::setfill('0') << range.h << "|" << std::setw(2) << std::setfill('0') << range.m << "|" << std::setw(2) << std::setfill('0') << range.s << " Average: " << std::setw(2) << std::setfill('0') << avg.h << "|" << std::setw(2) << std::setfill('0') << avg.m << "|" << std::setw(2) << std::setfill('0') << avg.s << " Median: " << std::setw(2) << std::setfill('0') << med.h << "|" << std::setw(2) << std::setfill('0') << med.m << "|" << std::setw(2) << std::setfill('0') << med.s;
    return ret_ss.str();
}