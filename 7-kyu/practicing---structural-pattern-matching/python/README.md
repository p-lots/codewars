...
___NOTE: Make Sure You Select Python 3.10 for Structural Pattern Matching.___
...
You can match with `sequence`, `or`, `guard`, `mapping`, using `as`,etc. Not just a replacement of `if ... elif ... elif ... else`, full details are in PEP636.
...
From [PEP635](https://peps.python.org/pep-0635/):
...
* If `arg` is a list with 2 or more elements, convert the first and last elements to integer if not yet integers, then return `first element divided by the last element (float division)`.
